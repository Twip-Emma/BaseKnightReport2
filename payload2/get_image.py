...
LastEditTime: 2023-11-13 16:59:57
...
def make_image(data, user_id):
    ...
    # 按照总伤害大小对用户进行排序
    sorted_users = sorted(data.items(), key=lambda x: sum(
        x[1].values()), reverse=True)

    # 计算总用户数和最大伤害值
    usernames = [user[0] for user in sorted_users]
    monsters = list(set(monster for user_data in data.values()
                    for monster in user_data.keys()))

    total_max_damage = max([sum(user_data.values())
                           for user_data in data.values()])
    image_width = min(total_max_damage / total_max_damage *
                      1000, 1000) + 200  # 加上左右边距
    image_height = (len(usernames) + 1) * (20 + 20) + 50

    image = Image.new(
        'RGB', (int(image_width), int(image_height)), (255, 255, 255))
    ...
    for monster, color in color_dict.items():
        legend_draw.rectangle(
            [(10 + legend_x_offset, 10), (30 + legend_x_offset, 10 + 10)], fill=color)
        legend_draw.text((40 + legend_x_offset, 10), monster,
    ...
    for username, user_data in sorted_users:
        ...
        for monster in monsters:
            ...
            bar_length = (damage / total_max_damage) * \
                (image_width - 200)  # 减去左边距宽度
            ...
            if damage > 0:
                ...
                text_y = y + 5  # 调整纵向位置
                draw.text((text_x, text_y), damage_text,
        ...
        if total_damage > 0:
            ...
            draw.text((total_damage_x, total_damage_y),
                      total_damage_text, fill=(0, 0, 0), font=font)
